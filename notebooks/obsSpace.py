...
def fit_rate(data, dz=1000):
    ...
    grouped = (
        grouped.reset_index()
    )  # reset the groupby column "height" from index to normal columns
    for idx, row in grouped.iterrows():
        print(
            f"{idx}, {row['height_bin']:.0f}, {row['rms_a']:.4f}, {row['rms_b']:.4f}, {row['fit_rate']:.4%}"
        )
            ...
            if dataset.groups[grp].groups:
                for nestgrp in dataset.groups[grp].groups:  # DiagnosticFlags
                    if varname in dataset.groups[grp].groups[nestgrp].variables:
                        data[nestgrp] = (
                            dataset.groups[grp].groups[nestgrp].variables[varname][:]
                        )
                ...
                if grp == "MetaData":
                    ...
                    data["ObsError"] = dataset.groups["ObsError"].variables[
                        "relativeHumidity"
                    ][:]
                    only_has_metadata = False
                elif (
                    varname == "brightnessTemperature"
                    and (grp == "ObsValue" or grp == "ObsValueAdj")
                    and "brightnessTemperature" in dataset.groups[grp].variables
                ):
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            raise AttributeError(
                f"'obsSpace' object has no attribute or variable '{name}'"
            )
